---
title: "Structural Nested Mean Models for Causal Inference"
...
$$
P(A=a|H=h) > 0 \text{ for all values of }h \tag{2}
$$
...
```{r,fig.cap="Illustration of confounding`. The more severely ill (high X1) are more likely to get treated leading to the situation where the average outcome is worse in the treated. Notice that positivity is violated in this illustration."}
...
```{r,fig.cap="Illustration of additive and nonadditive rank preservation"}
knitr::include_graphics("rankpreservation_v2.png")
```
