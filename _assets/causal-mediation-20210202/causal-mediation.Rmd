...
for (b in 1:B) {
  res[b,] <- mediation_scen0(N,alpha,beta,gamma) - true_res
}
...
# plot results
ggplot(res) +
  geom_density(aes(x=error,fill=estimand),alpha=0.2) +
  scale_fill_discrete(labels=c("Direct","Indirect","Total"),name="Effect") +
  labs(x = "Error\n(estimate - truth)",title="Mediation scenario 0",
...
true_res <- c(beta + alpha*gamma,beta,alpha*gamma)
for (b in 1:B) {
  res[b,] <- mediation_scen1(N,alpha,beta,gamma) - true_res
}
# prepare data for plot
res <- data.frame(res)
names(res) <- c("total","direct","indirect")
...
true_res <- c(beta + alpha*gamma,beta,alpha*gamma)
for (b in 1:B) {
  res[b,] <- mediation_scen2(N,alpha,beta,gamma) - true_res
}
# prepare data for plot
res <- data.frame(res)
names(res) <- c("total","direct","indirect")
